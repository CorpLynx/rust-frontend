...
remove_shell_integration() {
    ...
    if [ -f "$SHELL_RC" ] && grep -q "alias prometheus=" "$SHELL_RC"; then
        print_info "Removing shell alias from $SHELL_RC..."
        ...
        print_success "Shell alias removed (backup at ${SHELL_RC}.backup)"
    else
        print_info "No shell alias found"
    fi
}

# Remove man page
remove_man_page() {
    MAN_FILE="/usr/local/share/man/man1/prometheus-cli.1"
...
ask_about_ollama() {
    if command -v ollama >/dev/null 2>&1; then
        ...
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            print_info "To uninstall Ollama, run:"
...
# Main uninstall flow
main() {
    print_header "Prometheus CLI Uninstaller"
